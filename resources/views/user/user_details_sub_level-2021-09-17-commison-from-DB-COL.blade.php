...
<form method="POST" action="{{route('admin.user.details',['id' => $id])}}" accept-charset="UTF-8">
    ...
    <div class="col-md-4">
        <input name="daterange" type="text" placeholder="select Month" autocomplete="off">&nbsp;&nbsp;<input class="btn btn-primary text-white" type="submit" id="btn_submit" value="Apply" disabled="disabled">&nbsp;&nbsp;<a href="{{route('admin.user.details',['id' => $id])}}" class="btn btn-primary text-white">Reset</a>
    </div>
</form>
            ...
            @if(!$final_filter) <?php //if filter is not applied ?>
                <tr>
                    ...
                    <td style="font-size: 2rem">
                        <?php
                        if( ((int) $parentUser['subscriber_commision']) > 50) :
                            if( ((int) $parentUser['subscriber_commision']) >= 100) :
                                echo $amount; //$50
                            else:
                    ...
                    <td style="font-size: 2rem">
                        <?php
                        if( ((int) $parentUser['subscriber_commision']) > 100) :
                            $canWithDrawn = ( ((int) $parentUser['subscriber_commision']) - 100 );
                            if($canWithDrawn > 24 ):                                
                ...
                <tr>
                    ...
                    <td style="font-size: 2rem">
                        ...
                        if ($countNewRegistrations >= 0) {
                            if (((int)$parent['0']->subscriber_commision) == ($countNewRegistrations * 25) || $countNewRegistrations >= 0) {
                                echo '&#9989';
                            } else {
                                echo '&#x274C';
                            }

...
class PagesController extends Controller {
    ...
    public function dypage($slug, Request $request) {
        ...
        if (!isset($_COOKIE['admin-certificate'])) {
            ...
            if (in_array($slug, $restricted_slugs) && !session()->has("canWatch")) {
                $id = $request->id;
                if($request->has("type") && $request->input("type") == 'webinar') {
                    if (App::getLocale() == 'en') {
                        return redirect("pages/webinars-payments?id=$id")->with('error', ' Sorry! Please, enter your code');
                    } else {
                        return redirect("pages/paiements-pour-les-webinars?id=$id")->with('error', ' Sorry! Please, enter your code');
                    }
                }
        ...
        /** This code is webinar payments */
        if( $slug == "webinars-payments" || $slug == "paiements-pour-les-webinars"){
            $payment_html = view('blocks.payment')->render();
            $code_submit_html = view('blocks.code_submit')->render();
            $data['array']['pagesData']->content = str_replace("{payment}", $payment_html, $data['array']['pagesData']->content );
            $data['array']['pagesData']->content = str_replace("{codeSubmit}", $code_submit_html, $data['array']['pagesData']->content );
        }
    ...
    public function sendmail($slug, Request $request) {
        ...
        $sent = Mail::send('emails.certificate', array(
                    'name' => $request->name,
                    'email' => $request->email,
    ...
    public function token(Request $request) {
        ...
        if ($token) {
            ...
            if($request->has("type") && $request->input("type") == 'webinar') {
                return redirect("/register/$id?type=webinar");
            }
            ...
            $btoken = DB::table('training_video_payment')->where("token","=",$_POST['token'])->where("is_expired","=","YES")->where("address","!=","")->where("callback","!=","")->first();
            
            if(empty($btoken)):
                session()->put('canWatch', false);
                if($request->has("type") && $request->input("type") == 'webinar') {
                    if (App::getLocale() == 'en') {
                        return redirect('pages/webinars-payments?id=' . $id)->with('error', ' Sorry! Token you entered is invalid or expired.');
                    } else {
                        return redirect('pages/paiements-pour-les-webinars?id=' . $id)->with('error', ' Sorry! Token you entered is invalid or expired.');
                    }
                }
                return redirect('pages/videos?id=' . $id)->with('error', ' Sorry! Token you entered is invalid or expired.');
            else:
                
                $confirmations = ($btoken->confirmations) ? $btoken->confirmations : 0;
            
                if( $confirmations < 6 ):
                    session()->put('canWatch', false);
                    if($request->has("type") && $request->input("type") == 'webinar') {
                        if (App::getLocale() == 'en') {
                            return redirect('pages/webinars-payments?id=' . $id)->with('error', ' Payment confirmation is pending for your bitcoin transaction. Please keep your token safe and try again after some time.');
                        } else {
                            return redirect('pages/paiements-pour-les-webinars?id=' . $id)->with('error', ' Payment confirmation is pending for your bitcoin transaction. Please keep your token safe and try again after some time.');
                        }
                    }
                    return redirect('pages/videos?id=' . $id)->with('error', ' Payment confirmation is pending for your bitcoin transaction. Please keep your token safe and try again after some time.');
                else:
                    session()->put('canWatch', false);
                    if($request->has("type") && $request->input("type") == 'webinar') {
                        if (App::getLocale() == 'en') {
    ...
    public function videocode(Request $request) {
        ...
        if ($code) {
            ...
            if($request->has("type") && $request->input("type") == 'webinar') {
                if (App::getLocale() == 'en') {
                    return redirect("pages/webinars-payments?id=$id")->with('error', ' Sorry! Code you entered is invalid or expired.');
                } else {
                    return redirect("pages/paiements-pour-les-webinars?id=$id")->with('error', ' Sorry! Code you entered is invalid or expired.');
                }

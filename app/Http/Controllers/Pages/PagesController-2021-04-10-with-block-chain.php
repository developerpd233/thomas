    ...
    public function token(Request $request) {
        ...
        if ($token) {
            ...
            if(empty($btoken)):
                ...
                if( $confirmations < 6 ):
                    session()->put('canWatch', false);
                    return redirect('pages/videos?id=' . $id)->with('error', ' Your BTC transaction is not confirmed yet. Please wait for some time to confirm transaction and enter token again.');
                else:
                    session()->put('canWatch', false);
                    return redirect('pages/videos?id=' . $id)->with('error', ' Sorry! Token you entered is invalid or expired.');
                endif;            

...
class OnlinePaymentController extends Controller
{
    ...
    public function add()
    {
        ...
        $status = '';
        $cancelobj = DB::table('payments')->where('user_id', Auth::id())->where('cancel', 0)->where('payment_type', 'RECURRING')->first();
        if ($cancelobj) {
    ...
    public function addnew1()
    {
        ...
        $status = '';
        $cancelobj = DB::table('payments')->where('user_id', Auth::id())->where('cancel', 0)->where('payment_type', 'RECURRING')->first();
        $secrete_key = DB::table('payment_paypal')->where('userId', Auth::id())->where('isTokenUsed', 'no')->orderBy('id','desc')->first();
        ...
        return view('online-payment.addnew1', ['material' => $material, 'notNow' => $user->not_now, 'subsexists' => $subsexists, 'status' => $status, 'cancel' => $cancel, 'profile_id' => $profile_id,'secrete_key'=>($secrete_key->token ?? '')]);
    }
    ...
    public function activate()
    {
        ...
        $secrete_key = DB::table('payment_paypal')->where('userId', Auth::id())->where('isTokenUsed', 'no')->orderBy('id','desc')->first();
        return view('online-payment.addnew1', ['material' => $material, 'cancel' => $cancel, 'notNow' => $user->not_now, 'subsexists' => $subsexists, 'status' => $status, 'profile_id' => $profile_id,'secrete_key'=>($secrete_key->token ?? '')]);
    }
...
//dd($data['payment_type']);
    ...
    public function test(){
        $notUsedToken = DB::table('payment_paypal')
            ->where(array('endAt'=> null,'startAt'=>null,'isTokenUsed'=>'no','userId'=>659))->first();
        dd($notUsedToken->id);
    }

    public function verifyPaypalToken(){

        $formData = Input::all();
        $videocode = isset($formData['videocode']) ? $formData['videocode'] : '';
        $success = "Your payment received successfully.";
        $error = "Payment confirmation is pending for your paypal transaction. Please keep your token safe and try again after some time.";

        $qry = "select * from `payment_paypal` as pb where pb.`event`='CAPTURE' AND pb.`token`='".$videocode."' and pb.status='COMPLETED'";
        $info = DB::select($qry);
               ...
               // $subsctiption_expiry_type = (!empty($res[0]['subsctiption_expiry_type'])) ? $res[0]['subsctiption_expiry_type'] : "" ;
                ...
                if(!empty($response) ):
                    ...
                    if($is_active == "YES"):
                        ...
                        $endAt = new Carbon('first day of second month');

                        //update
                        $updVal = array( "startAt"=>$startAt, "endAt"=>$endAt, "isTokenUsed"=>"yes" );
                        $res = DB::table('payment_paypal')->where("id",$ID)->update($updVal);

                        //active user
                        $updVal = array( "is_active"=>"YES", "updated_at"=>date('Y-m-d H:i:s'),'ban_date'=>Carbon::now()->addMonth(2) );
                    ...
                    elseif($is_active == "NO" && $blockDays >= 60):
                        ...
                        //ban user
                        $updVal = array( "ban"=>"NO","is_active"=>"YES", "updated_at"=>date('Y-m-d H:i:s') );
                        $res = DB::table('users')->where("id",$userId)->update($updVal);
                    ...
                    if($res):
                        ...
                        return redirect('submit-bitcoin-tokens')->with('error', ' Payment confirmation is pending for your paypal transaction. Please keep your token safe and try again after some time!');
                    endif;
                //user response
                else:

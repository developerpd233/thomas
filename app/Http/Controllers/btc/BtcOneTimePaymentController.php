...
class BtcOneTimePaymentController extends Controller {
    ...
    public function banPaymentByBtcUsers(){               
        ...
        foreach ($userIDb as $user) {
            ...
            DB::table('users')->where('id', $user)->update(['is_active'=>'NO', 'ban_date'=>$banDate]);            
        }
        */
        ...
        //if user paied but not active token
        foreach ($bThisMonth as $payment) {
            $tMonth = DB::table('payments_btc')
                    ->where('userId', $payment->userId)
                    ->where('event', 'payment_detected')
                    ->where('confirmations', '>', 5)
                    ->where('isUsed', 'no') 
                    ->where('isTokenUsed', 'no')
                    ->get();
            
            if( count($tMonth) > 0 ):
                
                foreach ($tMonth as $arr) {    
                    
                    $ID = (!empty($arr->id)) ? $arr->id : "" ;
                    $userId = (!empty($arr->userId)) ? $arr->userId : "" ;
                    ...
                    //active user
                    $updVal = array( "is_active"=>"YES", "updated_at"=>date('Y-m-d H:i:s'), "ban"=>"NO" );
    ...
    public function verifyBitcoinToken(){
        ...
        if( !empty($res) ):
            ...
            if( $res[0]['isTokenUsed'] == 'no' ) :
                ...
                $qry = "select `id`, `is_active`, `ban`, `ban_date`, datediff(`ban_date`,now()) as `block_days` from `users` as u where u.`id`='$userId' ";
                $info = DB::select($qry);
                ...
                if(!empty($response) ):
                    ...
                    if($is_active == "YES"):
                        ...
                        //active user
                        $updVal = array( "is_active"=>"YES", "updated_at"=>date('Y-m-d H:i:s'), "ban"=>"NO", "ban_date"=>NULL );
                        $res = DB::table('users')->where("id",$userId)->update($updVal);
                    ...
                    elseif($is_active == "NO" && $blockDays >= 60):
                        ...
                        //ban user
                        $updVal = array( "updated_at"=>date('Y-m-d H:i:s'), "ban"=>"YES", "ban_date"=>date('Y-m-d') );
                        $res = DB::table('users')->where("id",$userId)->update($updVal);       

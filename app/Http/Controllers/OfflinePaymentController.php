...
class OfflinePaymentController extends Controller
{
    ...
    public function offline()
    {
        ...
        $secrete_key = DB::table('payment_paypal')->where('userId', $userID)->where('isTokenUsed', 'no')->orderBy('id','desc')->first();


        return view('offline-payment.off_add', ['material' => $material, 'notNow' => $user->not_now, 'subsexists' => $subsexists, 'status' => $status, 'cancel' => $cancel, 'profile_id' => $profile_id,'secrete_key'=>($secrete_key->token ?? '')]);
    }
    ...
    public function generateAddressForBtcPayment()
    {
        ...
        if(!empty($userID)):
            if(session()->has('btcOfflinePaymentAddress')):
                $s = session()->get('btcOfflinePaymentAddress');
                Session::put('btcOfflinePaymentAddress', $s);
            else:
                $resp = $this->btc_model->generateAddressForBtcPayment($userID);
                if( count($resp) > 0):
                    $s = $resp['address'];
                    Session::put('btcOfflinePaymentAddress', $s);
                    $data['address'] = $resp;
        ...
        return view('offline-payment.off_add', compact('data'));
